...
def form(data: dict | None = None, *, c: TestClient | None = None) -> dict:
    """Ajoute le jeton CSRF (cookie double-submit) aux données de formulaire."""
    c = c or client
...
def test_upload_creates_client_fiche() -> None:
    """POST /upload avec email/tel/adresse doit créer ou compléter la fiche Client."""
    from app.database import SessionLocal
    ...
    record(
        "POST /upload crée/complète fiche Client",
        ok,
