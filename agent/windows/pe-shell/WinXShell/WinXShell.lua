...
function App:onFirstShellRun()
  -- Forcer icones libres des le 1er affichage
  if Desktop then
    pcall(function()
      Desktop:AutoArrange(0)
      Desktop:SnapToGrid(0)
    end)
  end
end

function App:onShell()
  -- Restor-PC: icones deplacables (Refresh peut reactiver l'alignement)
  if Desktop then
    pcall(function()
      Desktop:AutoArrange(0)
      Desktop:SnapToGrid(0)
  ...
  -- Re-applique apres delai (id >= TID_USER = 20000)
  pcall(function()
    App:SetTimer(TID_USER + 1, 2000)
    App:SetTimer(TID_USER + 2, 5000)
    App:SetTimer(TID_USER + 3, 10000)
  end)
end
...
function App:onTimer(tid)
  ...
  if tid == (TID_USER + 1) or tid == (TID_USER + 2) or tid == (TID_USER + 3) then
    pcall(function() App:KillTimer(tid) end)
  end
end

-- Menu Demarrer classique WinXShell (textes FR patches dans WinXShell.exe)
-- Ne pas definir StartButton:onClick pour garder le menu natif.
