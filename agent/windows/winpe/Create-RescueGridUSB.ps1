...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File agent\windows\winpe\Create-RescueGridUSB.ps1 -TargetDrive E: -DashboardUrl "http://192.168.1.10:8000" -IncludeProject

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File agent\windows\winpe\Create-RescueGridUSB.ps1 -TargetDrive E: -Format -FileSystem FAT32 -WinPEBasePath C:\WinPE
#>
...
$agentRoot = Resolve-Path (Join-Path $scriptRoot "..") | Select-Object -ExpandProperty Path
$projectRoot = Resolve-Path (Join-Path $scriptRoot "..\..\..") | Select-Object -ExpandProperty Path

Write-Step "Copie des scripts agent"
$agentFiles = @("Invoke-RescueGrid.ps1", "Start-RescueGrid.ps1", "RescueGrid.GuiHelpers.ps1", "Setup-WinPEDesktop.ps1", "Start-WindowsInstall.ps1")
foreach ($file in $agentFiles) {
    $src = Join-Path $agentRoot $file
    if (Test-Path $src) {
...
$logoSrc = Join-Path $agentRoot "assets\restorpc_logo.png"
